...
\begin{align}
R_a(f) &= \frac{R_0}{2\pi j} \frac{1}{f-z} \\
z &= f_0 + \frac{j\gamma}{2\pi}
\end{align}
The Fourier transform $S(f)$ is given by Eq. \ref{eq:spec}.  Inserting the spectra into Eq. \ref{eq:midway} and taking the inverse Fourier transform should give $r_a(t) * s_a(t)$.  Note the inverse transform integral is only over positive frequencies, due to the $1+\sgn(f)$ factor.  Introducing the time derivative removes a factor of $(2\pi j f)$ from the numerator of the integrand.  The result is
\begin{multline}
r_a(t) * s_a(t) = \\ -j \sqrt{\frac{2}{\pi}} R_0 E_0 \sigma_t^3 \frac{d}{dt} \int_0^\infty \frac{e^{2\pi j ft} e^{-\frac{1}{2}\left(\frac{f}{\sigma_f}\right)^2}}{f-z}df
\end{multline}
Completing the square in the exponent, and letting $x = t/(\sqrt{2}\sigma_t)$, $y = f/(\sqrt{2}\sigma_f)$, and $z \to f_0/(\sqrt{2}\sigma_f)+j \gamma/(2\pi\sqrt{2}\sigma_f)$, we find
\begin{align}
r_a(t) * s_a(t) &= -j \sqrt{\frac{2}{\pi}} R_0 E_0 \sigma_t^3 \frac{dI_1}{dt} \\
I_1 &= e^{-x^2} \int_0^\infty \frac{e^{-(y-j x)^2}}{y-z} dy
\end{align}
The time derivative can be switched to a derivative with respect to $x$:
\begin{equation}
\boxed{
r_a(t) * s_a(t) = -j \frac{R_0 E_0 \sigma_t^2}{\sqrt{\pi}} \frac{dI_1}{dx}}
\end{equation}
Evaluating the derivative $dI_1/dx$ gives
\begin{equation}
\frac{dI_1}{dx} = 2j e^{-x^2} \int_0^{\infty} \frac{y e^{-(y-jx)^2}}{y-z}dy + 2 x I_1 - 2 x I_1
\end{equation}
The final two terms cancel, and the result is
\begin{align}
\Aboxed{r_a(t) * s_a(t) &= \frac{2R_0 E_0 \sigma_t^2}{\sqrt{\pi}} e^{-x^2} I_2(x,z)} \\
\Aboxed{I_2(x,z) &= \int_0^{\infty} \frac{y e^{-(y-jx)^2}}{y-z} dy}
\end{align}
\item Note that $I_2(x,0)$ has an exact solution:
\begin{align}
I_2(x,0) &= \frac{\sqrt{\pi}}{2}(1+\erf(jx)) \\
I_2(x,0) &= \frac{\sqrt{\pi}}{2} + j e^{x^2} D(x)
\end{align}
However, approximating $I_2(x,z) \approx I_2(x,0)$ drops the dependance on $\gamma$ and $f_0$.  This suggests expanding $I_2(x,z)$ in a Taylor series about $z=0$:
\begin{align}
I_2(x,z) &\approx I_2(x,0) + \sum_{n=1}^{\infty} z^n I_3(x,n) \\
I_3(x,n) &= \int_0^{\infty} y^{-n} e^{-(y-jx)^2} dy \\
I_3(x,n) &= \int_0^{\infty} y^{-n} e^{(x+jy)^2} dy
\end{align}
For example, when the lower limit is restricted from 0 to just $z$, the term $I(0,1)$ is
\begin{equation}
I(0,1) = \int_z^{\infty} y^{-1} e^{-y^2} dy = \frac{1}{2}E_1(z)
\end{equation}
\end{itemize}
