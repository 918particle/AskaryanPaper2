...
\begin{equation}
S(f) = E_0 (2\pi)^{3/2} j f e^{-2\pi^2 f^2 \sigma_t^2}
\end{equation}
...
\begin{equation}
S(f) = E_0 (2\pi)^{3/2} j f e^{-\frac{1}{2}\left(f/\sigma_f\right)^2}
\end{equation}
Inserting $S(f)$ into Eq. \ref{eq:sa_1}, $s_{\rm a}(t)$ is
\begin{equation}
s_{\rm a}(t) = \frac{E_0 \sigma_t^2 (2\pi)^{3/2}}{\pi} \frac{d}{dt}\int_0^{\infty} e^{-\frac{1}{2}\left(f/\sigma_f\right)^2} e^{2\pi j f t} df \label{eq:sa_3}
\end{equation}
\item Let the integral in Eq. \ref{eq:sa_3} be $I$, $k^2/4 = \frac{1}{2}\left(f/\sigma_f^2\right)$, and $x = t/(\sqrt{2}\sigma_t)$.  The real and imaginary parts of $I$ are
\begin{align}
\Re\lbrace I \rbrace &= \frac{\sigma_f}{\sqrt{2}} \int_0^{\infty} e^{-k^2/4}\cos(kx) dk \\
\Im\lbrace I \rbrace &= \frac{\sigma_f}{\sqrt{2}} \int_0^{\infty} e^{-k^2/4}\sin(kx) dk \\
\end{align}
The real part of $I$ is even, so it can be extended to $(-\infty,\infty)$ if it is multiplied by $1/2$.  The result is
\begin{equation}
\Re\lbrace I \rbrace = \sqrt{\frac{\pi}{2}} \sigma_f e^{-\frac{1}{2}\left(t/\sigma_t\right)^2}
\end{equation}
The imaginary part of $I$ is proportional to $D(x)$, known as \textit{Dawson's integral:}
\begin{equation}
\Im{I} = \sqrt{2}\sigma_f D(x)
\end{equation}
\item The overal analytic signal, $s_a(t)$, is
\begin{equation}
s_a(t) = -E_0 \left(t e^{-\frac{1}{2}\left(t/\sigma_t\right)^2} - \frac{j\sigma_t}{\sqrt{2\pi}} \frac{dD(x)}{dx}\right) \label{eq:sa_4}
\end{equation}
The signal envelope is $|s_a(t)|$:
\begin{equation}
\mathcal{E}_s(t) = E_0 \left(t^2 e^{-\left(t/\sigma_t\right)^2} + \frac{\sigma_t^2}{2\pi} \left(\frac{dD(x)}{dx}\right)^2\right)
\end{equation}
It is important to note that, though $D(x)$ is not evaluated analytically, a high-precision algorithm for computing $D(x)$ was given in \cite{10.1063/1.4822832}.  Note that $s_a(0) \neq 0$, since $D(x) = 1 - 2x D(x)$.
