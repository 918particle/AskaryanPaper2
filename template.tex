...
The time $t$ is the independent variable of the inverse Fourier transform of the equations in \cite{10.1103/physrevd.65.016003}. The delayed time is $t_{\rm r} = t-r/c$.  The Cherenkov angle $\theta_{\rm C}$ is set by the index of refraction, $n$, via $\cos\theta_{\rm C} = 1/n$.  The value often quoted for the RF bandwidth in ice ($n=1.78$) is 55.8 degrees.  The viewing angle $\theta$ is measured relative to the cascade axis, and the Askaryan radiation is concentrated when $\theta$ is close to $\theta_{\rm C}$.  The $n_{\rm max}$ parameter is the maximum excess negative charge in the cascade, and the overall RF amplitude is propotional to $n_{\rm max} a$.  Hanson and Hartig (HH) have shown that the parameter $p$ is related to the pulse width \cite{PhysRevD.105.123019}:
