...
\end{multline}
Let $u = x-y$, so that $du = -dy$ and note that $D(x)$ is an odd function.  The substitution gives
\begin{multline}
r_a(t) * \Im\lbrace s_a(t) \rbrace = \\ \frac{2}{\sqrt{\pi}} R_0 E_0 \sigma_t^2 \left(D(x) - z\int_{0}^{\infty} e^{zu} D(u-x) du \right)
\end{multline}
The integral is the Laplace transform of the shifted Dawson function, evaluated at $-z$.
