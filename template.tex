...
\item Equations \ref{eq:uncert}-\ref{eq:a_err} are used to reconstruct the natural logarithm of the UHE-$\nu$ cascade energy.  For Eq. \ref{eq:uncert}, $\sigma_t$ is measured from the optimized analytic envelope, $c$ and $\theta_{\rm C}$ are known constants, and an assumption must be made for $\Delta\theta$.  We do not compute $\Delta\theta$ from a single string
