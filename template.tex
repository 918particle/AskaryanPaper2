...
\begin{equation}
r_a(t) * \Im\lbrace s_a(t) \rbrace = \\ \frac{2}{\sqrt{\pi}} R_0 E_0 \sigma_t^2 \left(D(x) + k\mathcal{L}\lbrace D(u-x)\rbrace_k\right)
\end{equation}
%A more practical approach is to let the upper limit be $T \gg x$, and expand $D(u-x)$ in its Taylor series:
%\begin{align}
...
%\end{equation}
%Since $T\gg x$, the upper limit $T-x\gg 0$, and the lower limit $-x <0$. The exponential $\exp(-kx)$ is decaying in the right half-plane (since $\Re(k)>0$).  The integral may be approximated
%\begin{equation}
