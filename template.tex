...
\begin{abstract}
The detection of ultra-high energy neutrinos (UHE-$\nu$), with enegies above 10 PeV, has been a long-time goal of the astroparticle physics community.  A key strategy has been to deploy autonomous, radio-frequency (RF) detetectors in polar regions that rely on the Askaryan effect in ice for the neutrino signal.  The Askaryan effect occurs when the excess negative charge within a high-energy cascade radiates in a dense medium, and UHE-$\nu$ can induce such cascades that radiate in the radio-frequency (RF) bandwidth above thermal backgrounds.  To identify UHE-$\nu$ signals in future data from Askaryan-class detectors, analytic models of the Askaryan electromagnetic field have been created and compared to simulations and laboratory measurements.  These models have correctly described the Askaryan electromagnetic field, while leaving the effect of the RF detection channels on the field to simulation packages.  In this work, we present a fully analytic Askaryan model that accounts for the effect of an RF detection channel.  We provide formulas for the observed voltage trace, and for the Hilbert envelope of the trace.  We match the equations to data computed with NuRadioMC, a key Monte Carlo toolset in UHE-$\nu$ detection.  Our analytic model and NuRadioMC match, with correlation coefficients $>0.95$.  Using this correlation, UHE-$\nu$ signals can be distinguished from RF backgrounds with an efficiency of xxx.  Finally, we reconstruct the logarithm of the UHE-$\nu$ cascade energy with a precision of yyy using a single string of RF channels.
\end{abstract}
...
\end{figure}
\item A Monte Carlo data set was generated using NuRadioMC for UHE-$\nu$ interactions in a cylindrical ice volume with a depth of 0.65 km and a radius of 0.850 km.  The UHE-$\nu$ energies ranged from 10-100 PeV, with roughly equal numbers of UHE-$\nu$ in each 10 PeV bin.  The detector included a single string of 8 RF dipole antennas, a filtered passpand of [80,1000] MHz, and 256 samples per channel at a 1 GHz sampling rate.  The RF trigger responded when 3 of the 8 voltage traces exceeded $\pm 3$ times the $v_{\rm rms}$ of the thermal noise within 256 ns.  For each triggered UHE-$\nu$ interaction, the Hilbert envelope of the coherently summed waveform (CSW) was calculated and correlated against Eqs. \ref{eq:final}-\ref{eq:final2}.  A quick optimization over the whole data set yielded 0.15 GHz and 0.025 GHz for the $f_0$ and $\gamma$ parameters, respectively.  Each individual correlation was maximized by varying the just the $\sigma_t$ parameter, yielding an optimized analytical envelope for each event.  A thermal noise waveform reflective of the detector properties was also generated using NuRadioMC tools for each UHE-$\nu$ event.  The noise waveform was also correlated against the optimized analytical envelope.  The probability distributions for the correlation coefficients are shown in Fig. \ref{fig:fig3}.
\begin{figure}
\centering
...
\end{figure}
In Fig. \ref{fig:fig3}, the black circles represent the normalized histogram of the correlation coefficent between the optimized analytic envelope and the thermal noise waveform.  A function of the form $x^2 \exp(-0.5 x^2)$ was fit to the black circles on the interval [0.1,1.0], and is represented by the gray dashed line.  A function of the form $exp(-x)$ was fit to the noise distribution on the interval [0.28,1] to capture the limiting behavior, and is represented by the solid black line.  The solid gray line represents the signal distribution, which peaks at a correlation value of 0.96.  The vertical black dashed line represents a threshold value, 0.438.  For the simulated UHE-$\nu$ sample, 92\% of CSWs meet or exceed this correlation value when fit to an optimized analytic envelope.  Assuming the solid black line correctly describes the tail of the probability distribution function for noise correlation, a threshold of 0.438 is equivalent to 0.95 noise events per detector over a period of 5 years, at a noise trigger rate of 1 Hz.
