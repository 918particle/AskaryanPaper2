...
\item Equations \ref{eq:uncert}-\ref{eq:a_err} may be used to reconstruct the natural logarithm of the UHE-$\nu$ cascade energy, $\ln\Lambda$.  For Eq. \ref{eq:uncert}, $\sigma_t$ is measured from the optimized analytic envelope, $c$ and $\theta_{\rm C}$ are known constants, and an assumption must be made for $\Delta\theta$.  We will make the assumption that $\Delta\theta \approx \Delta\theta_{\rm rms}$.  Relying on a single string of RF channels makes calculating $\Delta\theta$ difficult.  However, the assumption is justified since the $\Delta\theta$ distribution is Gaussian.  The MC truth for $\Delta\theta$ in this analysis is shown in Fig. \ref{fig:fig5}, with a Gaussian fit ($\mu=55.6$ deg, $\sigma=5.77$ deg).  The rms and the $\sigma$ parameter are identical for Gaussian distributions, so we assume $\Delta\theta \approx 5.77$ deg.  The fractional error $\sigma_{\Delta\theta}/\Delta\theta$ is then set to 1.0, reflecting uncertainty in this parameter.  Solving Eq. \ref{eq:uncert} for $a$ gives
